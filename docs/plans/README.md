...
| [29](29-memory-borrow-roadmap.md) | Memory borrow roadmap | sanitized borrow plan: daemon readiness/parity, benchmark provenance, append-only supersession audit, context budget lanes, quality counters, structured filters, and temporal provenance | borrow mechanisms, not architectures |
| [30](30-autocontext-trace-curator.md) | AutoContext trace curator | turn `codex-extract --run --trace` JSONL into append-only review reports and playbook candidates; no auto-store, no fact DB, measured before promotion | analyze sessions without reviving the failed fact-index path |
| [31](31-candidate-unit-ir-eval.md) | Candidate-unit IR eval | recorded `claude -p` outputs for the generic `action(subject, object, verb)` candidate-unit prompt on the nine fail18 misses; deterministic scorer validates the saved `9/9` fixture without live provider calls | executable ambiguity / answer-variable discipline |
| [32](32-longmemeval-s-eval-discipline.md) | LongMemEval-S eval discipline | add gbrain-compatible session-level R@5, per-haystack vs global-pool protocols, locked reports, and claim gates so public benchmark claims are reproducible and bounded | Hippo-style eval honesty before SOTA claims |
