...
| [23](23-pi-harness-support.md) | pi coding-agent harness support | add the memory loop (recall-on-prompt, capture-on-stop, daemon health, transcript export) to **pi** (`@mariozechner/pi-coding-agent`); harness-agnostic canonical core + dual transport (daemon `POST /hook/{event}` + `simba hook-canonical` CLI) + thin bundled `simba.ts` bridge; `simba pi-install` ([plan](23-pi-harness-support-plan.md)) | third runtime |
