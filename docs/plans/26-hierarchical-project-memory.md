...
**Monorepo pain (kira):** 36 facts seeded under `/kira/api` don't recall from the
`/kira` root. Content-dedup (0.92) blocks dual-homing the same fact under two
...
```
recall(/kira/api) → {/kira/api} ∪ {/kira} ∪ {global}
recall(/kira)     →               {/kira} ∪ {global}
```
...
**This dissolves the dedup pain:** place a shared fact at `/kira` **once**; `api`
...
- **Path normalization / symlinks.** kira has symlinks (symlinked MEMORY.md);
  `.resolve()` both the stored paths and the chain, or ancestry breaks.
- **Bound at the git root** by default (don't leak `/Volumes/code/kirafin` across
  repos).
...
- **Collapse-to-git-root-id** (canonicalize memory store/recall like `rule add`):
  ...
  same-day stopgap to make the kira facts recall from root before the tree lands.)
