# Origins: a local-first evidence layer, and why store-raw

**Part 1 of 3 — DRAFT**

*simba is a pure-Python, local-first long-term memory layer for coding agents. This is
the first of three posts on how we built it and what we learned measuring it. Part 1 is
the origin story: the problem we started from, the architecture we landed on, and the one
design decision — store the raw turn, retrieve the complete evidence, let the agent
reason — that every later experiment ended up validating. Numbers in this post are
recall@k on LoCoMo and LongMemEval; the QA story comes in Parts 2 and 3.*
