# The neuro-symbolic bet, and what the data did to it

*Part 2 of 3 — DRAFT*

*simba is a pure-Python, local-first long-term memory layer for coding agents. Part 1
told the story of measuring our way to parity on LongMemEval. This post is about a bet we
made along the way — that propositional and first-order logic, fact schemas, Datalog,
solvers, and codegen would be the levers that beat plain store-and-retrieve on the hard
questions — and what happened when we measured it. Most of what follows is a record of
negative results. That's the point. The negatives told us something the wins couldn't:
**why** store-raw keeps winning.*
