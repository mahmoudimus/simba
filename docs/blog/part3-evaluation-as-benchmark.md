# Evaluation as a repeatable benchmark: how we earned the numbers

*Part 3 of 3 — DRAFT*

*simba is a pure-Python, local-first long-term memory layer for coding agents. The first
two posts covered the architecture and the retrieval pipeline. This one is about the part
nobody screenshots: the measurement program. It is the story of how we went from ad-hoc
scoring to a repeatable benchmark, and how that discipline — not a new architecture — took
simba from 0.561 to 0.823 on LongMemEval-S, landing at parity with the strongest
comparable open system we could re-run. We say parity, not a win, and the reason we can
only say parity is itself the point of this post.*
