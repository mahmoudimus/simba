...
class TestLexicalNovelty:
    def test_codes_symbols_camelcase_allcaps_are_novel(self) -> None:
        for t in ("50815", "verify.cpp", "%var_1DC", "sub_7FFD",
                  "TigressNotEqualSignBitRule", "INTERR", "Hodur", "tigress"):
    ...
    def test_common_english_words_are_not_novel(self) -> None:
        ...
        for t in ("how", "internal", "error", "control", "the", "block",
                  "consistency"):
...
class TestHighEntropyTerms:
    ...
    def test_unseen_token_is_high_entropy(self) -> None:
        ...
        terms = high_entropy_terms("hit INTERR 59999 today", self._dfmap(),
                                   n_docs=5000, min_bits=6.0)
        assert "59999" in terms

    def test_symbols_and_paths_survive_tokenization(self) -> None:
        terms = high_entropy_terms("crash in verify.cpp at %var_1DC", {},
                                   n_docs=5000, min_bits=6.0)
...
class TestExactBoost:
    def _pool(self):
        return [{"id": str(i), "content": c} for i, c in enumerate([
    ...
    def test_whole_word_only_no_substring(self) -> None:
        pool = [{"id": "a", "content": "code 508150 unrelated"},
