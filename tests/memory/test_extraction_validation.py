...
class TestHardValue:
    def test_hallucinated_amount_fails(self) -> None:
        r = validate_extraction("I paid $3,750 for the laptop",
                                "I paid $2,750 for the laptop last week")
        assert not r.ok and "hard_value" in r.failed

    def test_present_amount_passes(self) -> None:
        r = validate_extraction("the trip lasted 8 days",
                                "we camped and the trip lasted 8 days total")
        assert r.ok

    def test_hallucinated_date_fails(self) -> None:
        r = validate_extraction("baked sourdough on 2023-05-16",
                                "baked sourdough on 2023-05-20")
        assert not r.ok and "hard_value" in r.failed

    def test_comma_and_plain_number_normalize_equal(self) -> None:
        r = validate_extraction("reached 12000 points",
                                "she reached 12,000 points in the program")
        assert r.ok


class TestSupportOverlap:
    def test_unsupported_claim_fails(self) -> None:
        r = validate_extraction("the user adopted a golden retriever puppy",
    ...
    def test_faithful_subset_passes(self) -> None:
        r = validate_extraction("user is leading the cloud migration project",
                                "I am currently leading the cloud migration "
...
class TestPolarity:
    def test_invented_negation_fails(self) -> None:
        r = validate_extraction("the user does not like espresso",
    ...
    def test_matching_polarity_passes(self) -> None:
        r = validate_extraction("the user likes espresso",
...
class TestGateControl:
    def test_disabled_always_ok(self) -> None:
        r = validate_extraction("paid $9999 on 1999-01-01", "unrelated text",
                                enabled=False)
        assert r.ok and r.failed == []

    def test_multiple_failures_collected(self) -> None:
        r = validate_extraction("user did not pay $5000",
