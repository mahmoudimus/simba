...
def test_find_pending_matches_project_and_status(tmp_path: pathlib.Path) -> None:
    ...
    _meta(tmp_path, "s-d810", "/p/d810", "pending_extraction", "2026-06-04T02:00:00Z")
    # newest overall is d810, but we ask for simba -> must get simba, not d810
...
def test_cli_pending_scoped_by_project(tmp_path, monkeypatch, capsys) -> None:
    ...
    rc = m._cmd_transcript(["pending", "--project", "/p/d810"])
    out = capsys.readouterr().out
    assert rc == 0
    assert "s-d810" in out and "s-simba" not in out  # never the wrong project
