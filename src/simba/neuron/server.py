"""FastMCP server — registers all Neuron MCP tools and runs the server."""
...
def dispatch_agent(agent_name: str, ticket_id: str, instructions: str) -> str:
    ...
    """
    return simba.neuron.agents.dispatch_agent(agent_name, ticket_id, instructions)
...
def agent_status_update(ticket_id: str, status: str, message: str = "") -> str:
    ...
    """
    return simba.neuron.agents.agent_status_update(ticket_id, status, message)
...
def agent_status_check(ticket_id: str | None = None) -> str:
    ...
    """
    return simba.neuron.agents.agent_status_check(ticket_id)
...
def reload_server() -> str:
    ...
    """
    return simba.neuron.proxy.reload_server()
...
def run_server(root_dir: Path | None = None) -> None:
    ...
    simba.neuron.agents.register_sigchld_handler()
